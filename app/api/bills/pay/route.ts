 ...
 *
 * Failure semantics: if the on-chain payment succeeds but the DB write
 * fails, the response surfaces both — the chain truth survived but the
 * audit row didn't. Operator reconciles from on-chain history.
 */
...
export async function POST(req: Request): Promise<NextResponse> {
  ...
  // this is a privileged action, not a self-service family call.
  ...
  if (payErr || !payment) {
    ...
    // The on-chain payment succeeded; flag it loudly. The operator can
    // reconcile from Stellar Expert.
    return err(
      500,
      "db_error",
  ...
  if (updErr) {
    ...
    // bill_payment row exists; status didn't flip. Same posture as above.
    return err(
      500,
      "db_error",
